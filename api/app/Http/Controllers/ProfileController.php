...
class ProfileController extends Controller
{
	...
	{		
		$items = DB::select("
			...
			UPDATED_DATE,UPDATED_BY,ACTIVE_FLAG,role,sub_district,district,other,demarcate
			FROM profile
			/*where ACTIVE_FLAG='1'*/
			order by role
	...
	public function get_demarcate_and_admin(Request $request)
	{		
		$items = DB::select("
			SELECT 
			PROFILE_ID,TITLE,EMAIL,FIRST_NAME,LAST_NAME,POSITION,GENDER,
	...
	public function get_demarcate_list(Request $request)
	{		
		$items = DB::select("
			SELECT 
			PROFILE_ID,TITLE,EMAIL,FIRST_NAME,LAST_NAME,demarcate
			from profile
			where role=99 and ACTIVE_FLAG=1
			order by role
	...
	public function get_canvasser_only(Request $request)
	{		
		$items = DB::select("
			SELECT 
			PROFILE_ID,TITLE,EMAIL,FIRST_NAME,LAST_NAME,POSITION,GENDER,
			DATE_OF_BIRTH,ADDRESS,CREATED_DATE,CREATED_BY,	
			UPDATED_DATE,UPDATED_BY,ACTIVE_FLAG,role,sub_district,district,other,demarcate_id as demarcate_super_id,
			(select demarcate from profile where profile_id=demarcate_super_id) as demarcate
			
			FROM profile
			where role!=0 and role!=99
			order by role
	...
	public function updateNew(Request $request, $id)
	{
		...
		if ($validator->fails()) {
			...
			$item->demarcate=$request->demarcate;
			$item->demarcate_id=$request->demarcate_id;
