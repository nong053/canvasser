...
where b.booking_id=?
	...
	public function show($booking_id)
	{
		try {
			...
			$item = DB::select("
			SELECT 
			bv.booking_id,bv.fullname,bv.tel,
			bv.stay,bv.go_back_booking_flag,
			bv.booking_price_total,note,
			bv.date_go,bv.date_back,
			bv.boat_go,
			(select number_of_seats from vehicle v where bv.boat_go=v.vehicle_id) as number_of_seats_go,
			bv.boat_back,
			(select number_of_seats from vehicle v where bv.boat_back=v.vehicle_id) as number_of_seats_back,
			
			(select vehicle_number from vehicle where vehicle_id=bv.boat_go) as boat_number_go,
			(select vehicle_number from vehicle where vehicle_id=bv.boat_back) as boat_number_back,

			(select around_name from around_vehicle where id=bv.around_boat_go) as around_name_go,
			(select around_name from around_vehicle where id=bv.around_boat_back) as around_name_back,

			(select around_time from around_vehicle where id=bv.around_boat_go) as around_name_go_time,
			(select around_time from around_vehicle where id=bv.around_boat_back) as around_name_back_time,
			
			
			bv.around_boat_go,
			bv.around_boat_back,
			bv.seat_go,seat_back,
			bv.type_passenger,
			bv.ticket_vehicle_number
	...
	public function report_calendar_booking_vehicle(Request $request){
		...
		select booking_id,fullname,date_go,date_back,go_back_booking_flag
		from booking_vehicle
		order by date_go asc
		");
		return response()->json($items);
	}
	...
	public function around_vehicle(Request $request){
		...
		SELECT * FROM around_vehicle order by id asc
		");
		return response()->json($items);
	}
	...
	public function seat_go_back_reserve_bk(Request $request){
		$items = DB::select("
	...
	public function get_data_booking_by_booker(Request $request){
	
		$items = DB::select("
		SELECT 
bv.booking_id,bv.fullname,bv.tel,bv.stay,bv.go_back_booking_flag,
bv.booking_price_total,bv.note,bv.date_go,bv.date_back,
(select vehicle_number from vehicle where vehicle_id=bv.boat_go) as boat_go,
(select vehicle_number from vehicle where vehicle_id=bv.boat_back) as boat_back,

(select around_name from around_vehicle where id=bv.around_boat_go) as around_boat_go,
(select around_time from around_vehicle where id=bv.around_boat_go) as around_boat_go_time,
(select around_name from around_vehicle where id=bv.around_boat_back) as around_boat_back,
(select around_time from around_vehicle where id=bv.around_boat_back) as around_boat_back_time,

bv.seat_go,
bv.seat_back,
bv.type_passenger,bv.ticket_vehicle_number
 ...
 FROM booking_vehicle bv
	...
	public function get_data_booking_back_by_booker(Request $request){
...
bv.type_passenger,bv.ticket_vehicle_number
 ...
 FROM booking_vehicle bv
	...
	public function booking_vehicle_report(Request $request){
