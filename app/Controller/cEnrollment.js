...
var clearEnrollmentDataFn = function(){
	...
	$("#canvasserDemarcateText").prop("selectedIndex", 0);
...
var findOneEnrollmentDataFn = function(profileID){
	$.ajax({
		...
		success:function(data){
			...
			//$("select#canvasserDemarcateText select").val(data['demarcate_id']);
			$('select#canvasserDemarcateText  option[value="'+data['demarcate_id']+'"]').prop("selected", true);
...
var getEnrollmentDataFn = function(){
	...
	$.ajax({
		url:restURL+"/api/public/profile/get_canvasser_only",
		type:"get",
		dataType:"json",
		async:false,
		headers:{Authorization:"Bearer "+sessionStorage.getItem('galbalToken')},
...
var listGetDemarCateDataFn = function(data){
	var html="";
	$.each(data,function(index,indexEntry){
		if(index==0){
			//FIRST_NAME,LAST_NAME,demarcate
			html+="<option selected='selected' value='"+indexEntry['PROFILE_ID']+"'>"+indexEntry['FIRST_NAME']+" "+indexEntry['LAST_NAME']+" ("+indexEntry['demarcate']+")</option>";
		}else{
			html+="<option  value='"+indexEntry['PROFILE_ID']+"'>"+indexEntry['FIRST_NAME']+" "+indexEntry['LAST_NAME']+" ("+indexEntry['demarcate']+")</option>";
			//html+="<option value='"+indexEntry['military_rank']+"'>"+indexEntry['military_rank']+"</option>";
		}
		
	});

	$("#canvasserDemarcateText").html(html);
	$("#canvasserDemarcateText").prop("selectedIndex", 0);

}

var getDemarcateDataFn = function(){

	$.ajax({
		url:restURL+"/api/public/profile/get_demarcate_list",
		type:"get",
		dataType:"json",
		async:false,
		headers:{Authorization:"Bearer "+sessionStorage.getItem('galbalToken')},
		success:function(data){
			listGetDemarCateDataFn(data);
	...
	*/
	...
	$.ajax({
		...
		data:{
			...
			"other":$("#otherTxt").val(),
			"demarcate_id":$("#canvasserDemarcateText").val()
